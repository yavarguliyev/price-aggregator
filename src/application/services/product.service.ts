...
export class ProductService {
  ...
  async getStaleProducts() {
    const staleProducts = await this.productRepository.findStaleProducts(
  ...
  async checkAndMarkStaleProducts() {
    ...
    const result = await this.productRepository.markStaleProducts(
      this.stalenessThreshold,
    );

    if (result.count > 0) {
      this.logger.log(
        `Marked ${result.count} products as stale due to age (not updated since ${staleDate.toISOString()})`,
      );
      this.eventEmitter.emit("products.stale", {
        count: result.count,
        timestamp: new Date(),
      });
    }
