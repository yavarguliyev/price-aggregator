...
export class ProviderService {
  ...
  async fetchAllProviders() {
    try {
      const [provider1Products, provider2Products, provider3Products] =
        await Promise.all([
      ...
      this.eventEmitter.emit("providers.error", {
        error: error.message,
        timestamp: new Date(),
      });
    }
  ...
  private async fetchAndSaveProviderData(
    ...
    try {
      this.logger.debug(
        `Fetched ${products.length} products from ${providerName}`,
      );
      ...
      this.logger.debug(
        `Saved ${products.length} products from ${providerName}`,
      );
    } catch (error) {
      this.logger.error(
        `Error processing data from ${providerName}: ${error.message}`,
      );
      throw error;
    }
  }
